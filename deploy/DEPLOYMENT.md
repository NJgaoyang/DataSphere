...
DataSphere is distributed as `datasphere-0.1.0-SNAPSHOT.jar`. The metadata database is a dedicated MySQL database named `datasphere`; a fresh installation is created by Flyway `V1__baseline.sql`, and later incremental migrations are applied automatically.
...
From a source checkout, run `deploy/init-database.sql`. From a release bundle, run `config/init-database.sql`. Then create a dedicated MySQL account with privileges on `datasphere.*`. Use a new, empty `datasphere` database for a fresh installation.
