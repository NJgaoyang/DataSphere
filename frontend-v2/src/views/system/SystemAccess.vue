...
const auditPage=ref(1),auditPageSize=ref(50),auditTotal=ref(0),auditAction=ref(''),auditOperator=ref('')
...
async function load(){
  ...
  try{
    ...
    if(mode.value==='audit'){const result=await accessApi.auditLogs({action:auditAction.value||undefined,operator:auditOperator.value||undefined,page:auditPage.value,pageSize:auditPageSize.value});audits.value=result.items;auditTotal.value=result.total}
...
function changeAuditPage(page:number){auditPage.value=page;void load()}
watch(()=>route.path,()=>{auditPage.value=1;void load()});onMounted(load)
</script>
...
<template><div class="ds-page">
  ...
  <div v-else class="ds-card audit-card"><div class="audit-toolbar"><el-input v-model="auditAction" clearable placeholder="动作" @keyup.enter="searchAudit"/><el-input v-model="auditOperator" clearable placeholder="操作人" @keyup.enter="searchAudit"/><el-button @click="searchAudit">查询</el-button><span>共 {{auditTotal}} 条</span></div><table class="ds-table"><thead><tr><th>动作</th><th>资源</th><th>操作人</th><th>详情</th><th>时间</th></tr></thead><tbody><tr v-for="a in audits" :key="a.id"><td>{{a.action}}</td><td>{{a.resourceType}} {{a.resourceId||''}}</td><td>{{a.operatorName}}</td><td class="detail">{{a.detail||'—'}}</td><td>{{fmt(a.createdAt)}}</td></tr></tbody></table><div class="audit-pagination"><el-pagination background layout="prev, pager, next" :current-page="auditPage" :page-size="auditPageSize" :total="auditTotal" @current-change="changeAuditPage"/></div></div>
...
.audit-toolbar{height:52px;display:flex;align-items:center;gap:8px;padding:0 12px;border-bottom:1px solid var(--ds-border-soft)}.audit-toolbar :deep(.el-input){width:180px}.audit-toolbar>span{margin-left:auto;color:#98a2b3;font-size:12px}.audit-pagination{display:flex;justify-content:flex-end;padding:12px}.permission-note{border-color:#dfe8f3;border-radius:10px;background:#f6f9fd;color:#60728a}.permission-table{border:1px solid #e5ecf5;border-radius:10px;overflow:hidden}.permission-table th{height:40px;background:#fbfcfe;color:#78889f}.permission-table td{border-bottom-color:#eef2f7;color:#42566f}.permission-table tbody tr:hover td{background:#f7faff}
